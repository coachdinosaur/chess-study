...
test("all sixteen Markdown chapters are bundled as static source content", async () => {
...
test("the static output contains the interactive board and local engine assets", async () => {
  ...
  assert.match(source, /Catalan Atelier/);
  ...
  await Promise.all([
    access(new URL("dist/assets/pieces/mpchess/wK.svg", root)),
    access(new URL("dist/assets/pieces/mpchess/bQ.svg", root)),
    access(new URL("dist/stockfish/stockfish-18-lite-single.js", root)),
    access(new URL("dist/stockfish/stockfish-18-lite-single.wasm", root)),
    access(new URL("dist/app_icon_chess_study.png", root)),
  ]);
  assert.ok((await stat(new URL("dist/stockfish/stockfish-18-lite-single.wasm", root))).size > 1_000_000);
});
