...
export const Chessboard = memo(function Chessboard({ fen, flipped = false, onMove, lastMove }: { fen: string; flipped?: boolean; onMove?: (move: BoardMoveInput) => void; lastMove?: BoardMoveInput | null }) {
  ...
  return <div className={`board ${interactive ? "interactive" : ""}`} role={interactive ? "grid" : "img"} aria-label={boardLabel}>
    {ranks.flatMap((rank) => files.map((file) => {
      ...
      const content = <>
        {piece && <img data-color={piece.color} data-piece={piece.type} draggable={false} decoding="async" src={sharedOpeningAssetUrl(`assets/pieces/mpchess/${piece.color}${piece.type.toUpperCase()}.svg`)} alt={`${piece.color === "w" ? "White" : "Black"} ${piece.type}`} />}
    ...
    {promotion && <div className="promotion-picker" role="dialog" aria-modal="true" aria-label="Choose promotion piece">
      ...
      <div>{(["q", "r", "b", "n"] as const).map((piece) => <button type="button" key={piece} onClick={() => choosePromotion(piece)} aria-label={`Promote to ${piece === "q" ? "queen" : piece === "r" ? "rook" : piece === "b" ? "bishop" : "knight"}`}><img decoding="async" src={sharedOpeningAssetUrl(`assets/pieces/mpchess/${chess.get(promotion.from)?.color ?? "w"}${piece.toUpperCase()}.svg`)} alt="" /></button>)}</div>
